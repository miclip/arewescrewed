	...
	const faqs = [
		...
		},
		{
			...
			items: [
				...
				{
					q: 'Why don\'t AI provider profits flow back into company earnings?',
					a: 'Good eye. Microsoft owns 27% of OpenAI. Amazon owns ~15% of Anthropic. Google owns 100% of DeepMind. In reality, those provider profits would boost the parent companies\' earnings — and therefore their share prices and the investor model\'s income projections. We intentionally keep them separate for three reasons. First, DeepMind is already consolidated in Alphabet\'s reported financials, so folding it in again would double-count. Second, minority stakes (Microsoft/OpenAI, Amazon/Anthropic) have complex accounting — equity method income, mark-to-market adjustments — that varies by deal structure and is hard to model cleanly. Third, the provider engine uses a 21x economy scale factor to extrapolate from our 10-company portfolio to the full US economy, and mixing that scaled-up number back into individual company projections would conflate two different scopes. The result: the investor model slightly <em>understates</em> earnings for companies that own AI provider stakes. Think of the AI Providers tab as a separate lens showing where additional value accrues, not a gap in the core model.'
				},
				{
					q: 'What are the biggest weaknesses of this model?',
					a: 'Several. (1) It assumes AI capability continues improving roughly on trend — a major plateau would change everything. (2) It doesn\'t model government intervention (taxes, regulation, subsidies) which will absolutely happen. (3) It treats displacement as permanent — some displaced workers will find new roles. (4) It uses sector-level averages, but individual jobs within a sector will vary hugely. (5) It doesn\'t model inflation, interest rates, or market crashes. (6) AI provider profits from ownership stakes (e.g. Microsoft\'s 27% of OpenAI) aren\'t folded back into company earnings — the investor model slightly understates returns for these companies. It\'s a framework for thinking, not a crystal ball.'
				}
			]
		},
		{
		...
		},
		{
			...
			items: [
				...
				{
					q: 'Won\'t companies just keep workers and use AI to grow faster?',
					a: 'This is real — and it\'s exactly what happens first. Phase 1 of AI adoption is the velocity wave: same headcount, 3-5x the output, revenue booms. Some companies in genuine growth mode will hire <em>more</em> people during this phase because they\'re scaling into new markets faster than ever. But the velocity phase is a transition, not an equilibrium. Markets aren\'t infinite — you can\'t grow revenue 10x in a saturated industry just because your team ships faster. Once growth slows, the math flips: "we have 3x more people than we need to maintain this output" becomes a liability, not an asset. And even if established companies wanted to ride the velocity wave indefinitely, they can\'t control the ecosystem around them. A two-person startup with AI agents building a $100M ARR business creates pricing pressure that a 500-person competitor can\'t match. Foreign companies won\'t honor a gentleman\'s agreement to keep headcount high. Shareholders and boards see the margin opportunity and demand it. It only takes one or two companies in an industry to trigger the race — think Walmart being forced to match Amazon\'s efficiency or die. The velocity wave is real and it buys time, but it\'s Phase 1 of displacement, not an alternative to it.'
				},
				{
				...
				{
					q: 'Isn\'t this just another hype bubble like crypto?',
					a: 'It\'s a fair comparison — every major technology triggers a hype cycle, and the 1637 tulip mania crowd is never wrong to urge caution. <a href="https://en.wikipedia.org/wiki/Extraordinary_Popular_Delusions_and_the_Madness_of_Crowds" target="_blank" rel="noopener noreferrer">Extraordinary Popular Delusions and the Madness of Crowds</a> is full of legitimate innovations that still caused bubbles. But the analogy breaks down in a critical way. Crypto was a <em>financial</em> innovation searching for use cases — most people didn\'t <em>need</em> to use it, so its boundary was adoption. AI is a <em>cognitive automation</em> tool applied to work that already exists and already has massive economic value — companies desperately <em>want</em> to use it, so its boundary is capability. And that boundary keeps moving. Nobody needs AI to achieve singularity or suspend the laws of physics for the economics to bite. If a company can get 70% of the output for 5% of the labor cost, competitive pressure forces adoption. That\'s not magical thinking — it\'s the same math that drove offshoring, cloud migration, and every other cost arbitrage in business history. Will there be an AI correction? Almost certainly. Will the hype outrun reality in the short term? Probably. But even the conservative scenario in this model — a modest 25% displacement over 20 years — still affects tens of millions of workers. You don\'t need to believe the hype to take the math seriously.'
				},
				{
					q: 'Don\'t we still need humans who understand the code AI writes?',
					a: 'This is the "real programmers write assembly" argument of 2026. Every abstraction layer in computing history got the same pushback. Assembly programmers said you couldn\'t trust compilers. C programmers said garbage collection hid what the machine was really doing. Ops engineers said you needed to understand the hardware before moving to the cloud. Each time, the people who insisted on staying one level down got outrun by the people who embraced the abstraction and moved faster. Yes, someone still needs to understand systems at a deep level — but "someone" went from entire departments to a handful of specialists each time. AI code generation is the next abstraction layer. Today, you review AI output carefully. In two years, AI reviews its own output. In five, the entire concept of "reviewing code" may look as quaint as manually checking compiler output. The displacement doesn\'t require AI to be perfect — it requires AI to be good enough that one senior engineer supervising AI output replaces ten engineers writing code by hand.'
				},
				{
